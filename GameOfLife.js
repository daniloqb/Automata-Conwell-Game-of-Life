    ...
    this.sketch = function (p) {
      ...
      */
      ...
      p.mousePressed = function () {
        ...
        if (index) {
          if (found) {                         // verifica se a célula está na coleção de células ativas da grid
            grid.setCellState(index, dead);    // se estiver, marca como dead
            grid.activeCells.delete(index);    // exclui a célula da coleção de células ativas da grid
          } else {
            grid.setCellState(index, live);    // se não estiver ativa, marca como live
            grid.addActiveCell(index);         // adiciona na coleção de células ativas
          }
          grid.showCell(index);                // renderiza a célula selecionada para ajustar a tela.
        }
      };
